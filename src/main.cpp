...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in PayCon
    bool fStrictPayToScriptHash = true; // Always active in PayCon
...
bool CBlock::AcceptBlock()
{
	...
	/*	PayCon is not using a checkpoint server
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
	...
	/* PayCon is not using checkpoint server
	...
	// presstab PayCon: enable of disable staking based on block difficulty
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PayCon", CClientUIInterface::OK | CClientUIInterface::ICON_ETRKLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
		...
		/* PayCon does not use checkpoint server
